...
// Ping ping测试
func (permission *PermissionService) Ping(
	ctx context.Context,
	req *publicpb.Empty,
...
func (permission *PermissionService) GetUserPermission(
	ctx context.Context,
	req *permissionpb.GetUserPermissionRequest,
	resp *permissionpb.GetUserPermissionResponse,
) error {
	logger.Infof(
		"GetUserPermission %s||%v",
...
func (permission *PermissionService) AddUserPermission(
	ctx context.Context,
	req *permissionpb.AddUserPermissionRequest,
	resp *permissionpb.AddUserPermissionResponse,
) error {
	logger.Infof(
		"AddUserPermission %s||%v",
		req.BaseRequest.RequestInfo.Id,
		req.BaseRequest.UserInfo.UserId,
	)
	// 鉴权，只有超级管理员才可以进行此操作
	if !verify.Identify(verify.AddUserPermissionAction, req.BaseRequest.UserInfo.Levels) {
		logger.Info(
			"AdduserPermission permission forbidden: ",
			req.BaseRequest.RequestInfo.Id,
			", fromUserId: ",
			req.BaseRequest.UserInfo.UserId,
			", withLevels: ",
...
func (permission *PermissionService) RemoveUserPermission(
	ctx context.Context,
	req *permissionpb.RemoveUserPermissionRequest,
	resp *permissionpb.RemoveUserPermissionResponse,
) error {
	logger.Infof(
		"RemoveUserPermission %s||%v",
		req.BaseRequest.RequestInfo.Id,
		req.BaseRequest.UserInfo.UserId,
	)
	// 鉴权，只有超级管理员才可以进行此操作
	if !verify.Identify(verify.RemoveUserPermissionAction, req.BaseRequest.UserInfo.Levels) {
		logger.Info(
			"RemoveUserPermission permission forbidden: ",
			req.BaseRequest.RequestInfo.Id,
			", fromUserId: ",
			req.BaseRequest.UserInfo.UserId,
			", withLevels: ",
	...
	_, err := hpcdb.Transaction(
		...
		func(c context.Context, i ...interface{}) (interface{}, error) {
			...
			err := permission.userpLogic.RemoveUserPermission(
				c,
				int(req.GetUserid()),
				verify.Level(req.GetLevel()),
			)
			if err != nil {
...
func (permission *PermissionService) AddPermission(
	ctx context.Context,
	req *permissionpb.AddPermissionRequest,
	resp *permissionpb.AddPermissionResponse,
) error {
	logger.Infof(
		"AddPermission %s||%v",
		req.BaseRequest.RequestInfo.Id,
	...
	if !verify.Identify(verify.AddPermission, req.BaseRequest.UserInfo.Levels) {
		logger.Info(
			"AddPermission permission forbidden: ",
			req.BaseRequest.RequestInfo.Id,
			", fromUserId: ",
			req.BaseRequest.UserInfo.UserId,
			", withLevels: ",
...
func NewPermission(
	...
	userpermissionLogic *logic.UserPermission,
	permissionLogic *logic.Permission,
) *PermissionService {
