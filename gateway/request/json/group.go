...
func (cgp *CreateGroupParam) Validator() validator.StructLevelFunc {
	return func(sl validator.StructLevel) {
		...
		if len(data.GroupName) == 0 || len(data.GroupName) > 64 {
			sl.ReportError(
				reflect.ValueOf(data.GroupName),
				"groupName",
				"groupName",
				"binding",
				"groupName length error",
			)
		}
		if len(data.QueueName) == 0 || len(data.QueueName) > 64 {
			sl.ReportError(
				reflect.ValueOf(data.QueueName),
				"queueName",
				"queueName",
				"binding",
				"queueName length error",
			)
		}
	}
}
...
func (c *CreateJoinGroupApplyParam) Validator() validator.StructLevelFunc {
	return func(sl validator.StructLevel) {
		...
		if data.ApplyGroupID <= 0 {
			sl.ReportError(
				reflect.ValueOf(data.ApplyGroupID),
				"applyGroupID",
				"applyGroupID",
				"binding",
				"applyGroupID error",
